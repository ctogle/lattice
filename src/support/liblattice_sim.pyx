...
class lattice_site(object):
    ...
    def __init__(self, args, pop = ''):
        ...
        self.lattice = args[2]
        self.topology = site_topology(self.position, self.lshape)
                
    def resolve_agents(self, lattice, pop):
        local_ags = self.agents
...
class lattice(object):
    ...
    def pick_agent_to_act(self):
        ...
        return cbindex(prop_table, self.agent_count)
...
def simulate(sys_string = def_string):
    ...
    _lattice_ = lattice(
        dims = lattice_dims, ax_length = lattice_size, 
    ...
    start_time = ti.time()
    dead_eco_check = lambda : _lattice_.agent_count > 0
    #dead_eco_check = lambda : state[1] > 0
    dead_eco = dead_eco_check()
    while capture_count < total_captures and dead_eco and not timed_out:
